...
def build_model(info_dict, labels, model_name_str, base_learning_rate = 0.00001, dropout_val = None, class_numbers = None, IMG_SIZE = 96):
...
def label_naming_shorthand_dict(name_key = None):
    label_naming_shorthand_dict = {'[0, 1, 2, 3, 4, 5]- (no touch, touch, onset, one after onset, offset, one after offset)': 'on-off_set_and_one_after',
...
def get_automated_model_info(BASE_H5, image_source_h5_directory_ending):
    ...
    print('\nFOR IMAGES, Ind 0 is test set, 1 is train set, 2 is val set')
    ...
    print('\nFOR LABELS, Ind 0 is test set, 1 is train set, 2 is val set')
...
make_labels_and_h5s = False # DETERMINES if you re run the data transformation form the single frames into the regular, 3lag and diff_3lag

label_key_name_list = label_naming_shorthand_dict()
utils.print_list_with_inds(label_key_name_list)
label_key_name = label_key_name_list[0] # DETERMINES THE LABEL SOURCE choose the ind based on the print out

local_dict = make_initial_folder(all_models_directory, unique_h5_train_val_dir)
info_dict = info_dict_wrapper(info_dict, local_dict)

BASE_H5 = info_dict['all_models_directory'] + os.sep + info_dict['unique_h5_train_val_dir']
local_dict = get_automated_model_info(BASE_H5, image_source_h5_directory_ending)
info_dict = info_dict_wrapper(info_dict, local_dict)

local_dict = copy_over_new_labels(label_key_name, info_dict['image_h5_list'], info_dict['label_h5_list'])
info_dict = info_dict_wrapper(info_dict, local_dict)

if make_labels_and_h5s: # make all the h5 files with lag and subtraction and regular
    utils.make_all_H5_types(info_dict['base_dir_all_h5s']) # auto generate all the h5 types using a single set of flat (no color) image H5
    utils.make_alt_labels_h5s(info_dict['base_dir_all_h5s']) # auto generate the different types of labels
"""$$$$$$$$$$$$$$$$$$$$$$$$$$$      make model     $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$"""
...
model_save_dir = make_model_save_directory(info_dict)
model, class_weights, info_dict = build_model(info_dict, labels, model_name_str, base_learning_rate = base_learning_rate, dropout_val = dropout_val)
...
def basic_callbacks(save_checkpoint_filepath, monitor = 'val_loss', patience = 10,
    ...
    add_path_name ="{val_loss:.8f}_{epoch:04d}_cp.hdf5"
